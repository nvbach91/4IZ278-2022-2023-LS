    ...
    if (!isset($_SESSION["email"])) {
        ...
        if ($_GET['week'] == '0') {
            ...
            header('Location: main_booking_next.php');
               ...
               $data = $conn->prepare("INSERT INTO `_dmp_booking` (`ID`, `username`, `day`, `time`,`week`,  `_dmp_users_ID`, `_dmp_booking_cells_ID`, `oauth`) VALUES (NULL, ?, ?, ?, ?, ?, ?, ?)");
        $data->execute(['googleAuth', $_GET['day'], $_GET['time'], $_GET['week'], 0, $value['ID'], $_SESSION['email']]);
        if ($_GET['week'] == '0') {
            header('Location: main_booking.php');
        } else {
            header('Location: main_booking_next.php');
