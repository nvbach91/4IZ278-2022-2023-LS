			...
			<div class="card col-xl-6">
				<div class="card-body">
					...
					<div class="mt-4 w-full justify-content-center">
						<form id="productEdit" method="POST" action="<?php echo $GLOBALS['request']; ?>?id=<?php echo $_GET['id']; ?>&action=<?php echo $_GET['action']; ?>">
							...
							<div class="mb-4">
								<select id="category" name="category" class="select <?php echo isset($fieldStatuses['category']) ? 'is-invalid' : '' ?>" tabindex="2">
									<option value="" selected>Please select category</option>
									<?php foreach ($categories as $categorySelect): ?>
										<option value="<?php echo $categorySelect['id_category'] ?>" <?php echo $category == $categorySelect['id_category'] ? 'selected' : '' ?>><?php echo $categorySelect['name'] ?></option>
									<?php endforeach; ?>
								</select>
