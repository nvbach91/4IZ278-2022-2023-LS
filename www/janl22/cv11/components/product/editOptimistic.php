...
$_SESSION['productEditStartTime'] = time();
...
if ($formSubmitted) {
	...
	if (empty($fieldStatuses)) {
		...
		$editedProductLastUpdate = $ProductsDB->fetchById(intval($_GET['id']))['last_update'];

		if ($_SESSION['productEditStartTime'] > $editedProductLastUpdate) {
			...
			$fieldStatuses['itemAlreadyEdited'] = new StatusMessage('The item has been updated by another user! Please <a href="editProductOptimistic?id=' . $_GET['id'] . '">refresh the page</a> and try it again. Warning: All changes will be lost!','error');
		}
				...
				<div class="card-body">
					...
					<div class="mt-4 w-full justify-content-center">
						<form id="productEdit" method="POST"
							...
							<?php if (!isset($fieldStatuses['itemAlreadyEdited'])): ?>
