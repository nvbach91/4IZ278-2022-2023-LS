...
document.querySelectorAll(".editProduct-btn").forEach(editProductBtn => {
    editProductBtn.addEventListener("click", e => {
        ...
        .catch(err => {
            document.querySelector("nav").insertAdjacentHTML("afterend", `<p style='color:red; font-size: 1.6em;'>${err}</p>`)
    })
...
document.querySelector("#editProductForm").addEventListener("submit" , e => {
    ...
    xhr.send(`product_id=${document.querySelector("#editProductForm input[name='product_id']").value}&name=${document.querySelector("#editProductForm input[name='name']").value}&price=${document.querySelector("#editProductForm input[name='price']").value}&img=${encodeURIComponent(document.querySelector("#editProductForm textarea[name='img']").value)}&category=${document.querySelector("#editProductForm input[name='category']").value}&token=${document.querySelector("#editProductForm input[name='token']").value}`);
...
document.querySelector("#addCategoryForm").addEventListener("submit" , e => {
    ...
    + "&token=" + document.querySelector("#addCategoryForm input[name='token']").value);
...
//     .catch(err => console.log(err));
 ...
 document.querySelector("#addProductForm").addEventListener("submit" , e => {
    ...
    xhr.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
    xhr.send(`name=${document.querySelector("#addProductForm input[name='name']").value}&price=${document.querySelector("#addProductForm input[name='price']").value}&img=${document.querySelector("#addProductForm input[name='img']").value}&category=${document.querySelector("#addProductForm input[name='category']").value}&token=asdasd`);
