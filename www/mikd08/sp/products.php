...
<main>
    <div id="categories">
        <h2>Categories</h2>
        <div class="category">
            <a href="index.php" class="category_link custom-link">All</a>
        </div>
        ...
        <?php foreach($categories as $category):?>
            <div class="category">
                <a href="index.php?category=<?php echo htmlentities($category["category_name"]) ?>" class="category_link"><?php echo htmlentities($category["category_name"]) ?></a>
                <?php if(isset($_SESSION["isAdmin"]) && $_SESSION["isAdmin"] == "true"):?>
                <span style="display: none;">
                    <form action="deleteCategory.php" id="delete_category<?php echo htmlentities($category["category_name"]) ?>" method="POST">
        ...
        <div class="card" style="min-width: 200px; width: calc(33% - 40px); margin: 20px;">
            ...
            <div class="card-body">
                <?php if(isset($_SESSION["isAdmin"]) && $_SESSION["isAdmin"] == "true"):?>
                    ...
                    <form action="deleteProduct.php" method="POST" id="delete_product<?php echo htmlentities($product["product_id"]) ?>" style="display:none;">
                        <input type="text" name="product_id" value="<?php echo htmlentities($product["product_id"]) ?>">
                    </form>
                ...
                <?php  else:?>
                    ...
                    <?php  else:?>
                        <a href="buy.php?product_id=<?php echo htmlentities($product["product_id"])?>" class="card-link addToCart-link" id="buy-btn">Add to cart</a>
                    <?php  endif?>
                <?php  endif?>
            </div>
        </div>
...
<nav aria-label="...">
    <ul class="pagination">
        <?php for($i = 0; $i < $numPages; $i++){?>
            <li class="page-item"><a class="page-link" href="<?php echo "index.php?limitFrom=".$i*$limit.(count($_GET) > 0 ? "&$params": "")?>"><?php echo $i + 1 ?></a></li>
        <?php }?>
    </ul>
</nav>