...
if ('POST' == $_SERVER['REQUEST_METHOD']) {
    ...
    $stmt = $db->prepare('INSERT INTO cv10_users(name, email, password) VALUES (:name, :email, :password)');
    $stmt->execute([
        'name' => $name, 
        'email' => $email, 
    ...
    $stmt = $db->prepare('SELECT user_id FROM cv10_users WHERE email = :email LIMIT 1'); //limit 1 jen jako vykonnostni optimalizace, 2 stejne maily se v db nepotkaji
    $stmt->execute([
        'email' => $email
    ]);
