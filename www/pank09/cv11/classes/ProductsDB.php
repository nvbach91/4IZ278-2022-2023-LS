...
class ProductsDB extends Database
{
    ...
    {
        $statement = $this->db_conn->prepare("UPDATE `$this->db_table` SET `name` = :name, `price` = :price, `description` = :description, `img` = :img, `updated_at` = now() WHERE `$this->db_table_pk` = :id");
        $args['id'] = $id;
        return $statement->execute($args);
    }

    public function updateLock($id, $args)
    {
        $statement = $this->db_conn->prepare("UPDATE `$this->db_table` SET `lock_opened_at` = :lock_opened_at, `lock_opened_by` = :lock_opened_by WHERE `$this->db_table_pk` = :id");
        $args['id'] = $id;
        return $statement->execute($args);
    }
