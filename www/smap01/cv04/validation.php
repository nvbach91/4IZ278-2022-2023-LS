...
if ($formIsSubmitted) {
    ...
    if (empty($password) || empty($passwordVal)) {
        $message = 'Both password and passsword confirmation must not be empty';
        array_push($errors, $message);
    } else if ($password != $passwordVal) {
        $message = 'Password confirmation must be the same as password entered';
        array_push($errors, $message);
    }

    //Nacist data do promenne
    if (empty($error)) {
        array_push($errors, registerNewUser($errors));
        if (empty($errors)) {
            $to = $email;
            $subject = "Successful registration";
            $txt = "Good news! You registration was a success.";
