...
//First enter tests
if(!isset($_SESSION)||!isset($_SESSION['books'])||!isset($_COOKIE)||!isset($_COOKIE['user_email'])||!$usersDB->userExists($_COOKIE['user_email'])){
...
$userId=$usersDB->getUserID($_COOKIE['user_email']);
if($ordersDB->placeOrder(htmlspecialchars($userId), $_SESSION['books'], htmlspecialchars($usersDB->getUser($userId)['user_address']), date("Y-m-d H:i:s"))==NULL){
    sendConfirmation($userId, $usersDB);
    session_destroy();
}
header('Location: ../profile.php?user_id='.$usersDB->getUserID($_COOKIE['user_email']));
exit;
...
function sendConfirmation($userId, $usersDB){
    $to      = $usersDB->getUser($userId)['user_email'];
