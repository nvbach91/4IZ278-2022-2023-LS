...
class OrdersDB
{
    ...
    function placeOrder($user_id, $items, $orderAddress, $orderDate)
    {
        try {
            ...
            $statement->execute([':order_date' => htmlspecialchars($orderDate), ':order_user_id' => htmlspecialchars($user_id), ':order_address' => $orderAddress]);
            ...
            foreach ($items as $item) {
                ...
                $statement->execute([
                    ':price' => (float)$productsDB->getBook($item['book_id'])['price']*$item['book_count'],
