...
if (!empty($_POST) && isset($_POST['book_id']) && isset($_POST['name']) && isset($_POST['description']) && isset($_POST['price']) && isset($_POST['authors']) && isset($_POST['thumbnail']) && $usersDB->getUserPrivilege($_COOKIE['user_email']) >= 2) {
    ...
    if (strlen($result) == 0) {
        $result = $productsDB->updateBook(htmlspecialchars($_POST['book_id']), htmlspecialchars(filter_var($_POST['name'], FILTER_UNSAFE_RAW)), htmlspecialchars(filter_var($_POST['authors'], FILTER_UNSAFE_RAW)), htmlspecialchars(filter_var($_POST['description'], FILTER_UNSAFE_RAW)), htmlspecialchars($_POST['thumbnail']), htmlspecialchars($_POST['price']));
        if ($result == null) {
