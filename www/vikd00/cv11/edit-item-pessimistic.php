...
{
    $statement = $pdo->prepare('UPDATE cv09_goods SET edit_start_time = :edit_start_time, editing_user_id = :editing_user_id WHERE good_id = :good_id');
...
{
    $statement = $pdo->prepare('UPDATE cv09_goods SET edit_start_time = NULL, editing_user_id = NULL WHERE good_id = :good_id');
    $statement->execute(['good_id' => $good_id]);
}
...
if (!empty($_GET['good_id'])) {
    ...
    if ($item['editing_user_id'] !== null && $item['editing_user_id'] != $_SESSION['user']['id'] && getCurrentTimestamp() - $item['edit_start_time'] < 300) {
        $_SESSION['message'] = "The item is currently being edited by another user. Please try again later.";
        header('Location: ./index.php');
        exit;
    } else {
        lockItem($pdo, $item['good_id'], $_SESSION['user']['id']);
    }
}
