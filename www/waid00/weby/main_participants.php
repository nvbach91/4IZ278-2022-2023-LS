...
if ($_SESSION["privilege"] >= '2' AND !isset($_GET["day"])) {
  $data = $conn->prepare("SELECT `username`, `mail`, `birth_date`, `gender`, `privileges`, `forename`, `surname`, `confirmed`, `wallet`, `banned` FROM `_dmp_users` WHERE `username` != ? ORDER BY `surname` ASC");
